module Riopro
  module KillBill
    module Bank
      class Itau < Riopro::KillBill::Bank::Base
        ...
        def initialize(options = {})
          super
          self.bank = "itau"
          self.bank_number = 341
          self.bank_number_cd = 7
          self.portfolio = 175 # carteira "sem registro"
        ...
        def build_barcode(verifyer_cd = "")
          barcode = "#{self.bank_number}#{self.currency}#{verifyer_cd}#{@factor}#{@formatted_value}#{self.portfolio}"
          barcode << "#{self.our_number}#{self.calculate_our_number_cd}#{self.agency}"
          barcode << "#{self.account}#{self.account_cd}000"
          barcode
        end

        # Calculates our number check digit according to Itaú criteria
        def calculate_our_number_cd
          if %w(126 131 146 150 168).include?(self.portfolio.to_s)
            module10("#{self.portfolio}#{self.our_number}");
          else
            module10("#{self.agency}#{self.account}#{self.portfolio}#{self.our_number}");
          end
        end

        # Calculates account check digit according to Itaú criteria
